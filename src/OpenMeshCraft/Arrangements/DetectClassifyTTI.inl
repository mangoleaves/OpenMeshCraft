                                                     ...
                                                     TTIHelper &hb)
{
	...
	phmap::flat_hash_set<CoplanarEEI, Hasher> coplanar_inter_list;
                                                       ...
                                                       TTIHelper &hb)
{
	...
	// intersection list
	phmap::flat_hash_set<index_t>             inter_list;
	// intersection vertex list for final symbolic edge creation
	...
	if (eav0_wrt_tb == Sign::ZERO && eav1_wrt_tb == Sign::ZERO)
	{
		...
		auto  &cil = coplanar_inter_list; // a short name :)
		size_t hi  = 0;                   // has intersections ?
		...
		if (eav0_wrt_sector == 0) // previous edge of ea intersects tb
			hi += classify_coplanar_edge_intersections(ha, (ea + 2) % 3, hb, il, cil);
		size_t eav1_wrt_sector = get_vtx_wrt_sector(ha, eav1, hb, eb);
		if (eav1_wrt_sector == 0) // next edge of ea intersects tb
			hi += classify_coplanar_edge_intersections(ha, (ea + 1) % 3, hb, il, cil);
		...
		if (ebv0_wrt_sector == 0) // previous edge of eb intersects ta
			hi += classify_coplanar_edge_intersections(hb, (eb + 2) % 3, ha, il, cil);
		size_t ebv1_wrt_sector = get_vtx_wrt_sector(hb, ebv1, ha, ea);
		if (ebv1_wrt_sector == 0) // next edge of eb intersects ta
			hi += classify_coplanar_edge_intersections(hb, (eb + 1) % 3, ha, il, cil);
		...
		hi += classify_coplanar_edge_intersections(ha, ea, hb, il, cil);
		hi += classify_coplanar_edge_intersections(hb, eb, ha, il, cil);
	...
	{ // one edge of `ta` on the support plane of `tb`
		...
		{
			if (classify_coplanar_edge_intersections(ha, copl_ea, hb, inter_list,
			                                         coplanar_inter_list))
	...
	{ // one edge of `tb` on the support plane of `ta`
		...
		if (get_vtx_wrt_sector(hb, copl_vb, ha, ea) == 0)
		{
			if (classify_coplanar_edge_intersections(hb, copl_eb, ha, inter_list,
			                                         coplanar_inter_list))
				g.addCoplanarEdge(ha.t_id, hb.e_id[copl_eb]);
		}
		else
	...
	OMC_EXPENSIVE_ASSERT(
	  vtx_list.size() <= 2,
	  "more than 2 intersection points between 2 non-coplanar triangles.");
...
void DetectClassifyTTI<Traits>::check_TTI_separate(TTIHelper &ha, TTIHelper &hb)
{
	...
	bool coplanar_tris = _allCoplanarEdges(orAB);
	...
	else if (_singleCoplanarEdge(orAB, edge_id))
	{
		// CASE: a single edge of ta is coplanar to tb    (e.g. orAB: 1 0 0)
		if (coplanar_seg_tri_do_intersect(ha, edge_id, hb))
		{
			if (classify_coplanar_edge_intersections(ha, edge_id, hb, inter_list,
			                                         coplanar_inter_list))
	...
	else if (_vtxInPlaneAndOppositeEdgeOnSameSide(orAB, vtx_id))
	{
		...
		classify_coplanr_vtx_intersections(ha, vtx_id, hb, inter_list, vtx_list);
	...
	if (coplanar_tris)
	{
		orBA[0] = Sign::ZERO, orBA[1] = Sign::ZERO, orBA[2] = Sign::ZERO;
	}
	else
	{
	...
	else if (_singleCoplanarEdge(orBA, edge_id))
	{
		// CASE: a single edge of tB is coplanar to tA    (e.g. orBA: 1 0 0)
		if (coplanar_seg_tri_do_intersect(hb, edge_id, ha))
			if (classify_coplanar_edge_intersections(hb, edge_id, ha, inter_list,
			                                         coplanar_inter_list))
				g.addCoplanarEdge(ha.t_id, hb.e_id[edge_id]);
	}
	else if (_vtxInPlaneAndOppositeEdgeOnSameSide(orBA, vtx_id))
	{
		...
		classify_coplanr_vtx_intersections(hb, vtx_id, ha, inter_list, vtx_list);
	}
	...
	if (coplanar_tris)
	{
		OMC_EXPENSIVE_ASSERT(
		  inter_list.size() <= 6,
		  "more than 6 intersection points in coplanar triangles");
	}
	else
	{
		OMC_EXPENSIVE_ASSERT(
                                                     ...
                                                     TTIHelper &hb, index_t eb)
{
	...
	index_t eb0 = eb, eb1 = (eb + 1) % 3;

	Sign ea0_wrt_eb = get_v_wrt_seg(ha, ea0, hb, eb);
	Sign ea1_wrt_eb = get_v_wrt_seg(ha, ea1, hb, eb);
	Sign eb0_wrt_ea, eb1_wrt_ea;
	if (ha.t_nmax == hb.t_nmax)
	{
		eb0_wrt_ea = get_v_wrt_seg(hb, eb0, ha, ea);
		eb1_wrt_ea = get_v_wrt_seg(hb, eb1, ha, ea);
	}
	else
	{
		eb0_wrt_ea = OrientOn2D()(ha.v[ea], ha.v[ea1], hb.v[eb0], hb.t_nmax);
		eb1_wrt_ea = OrientOn2D()(ha.v[ea], ha.v[ea1], hb.v[eb1], hb.t_nmax);
	}
                                                                 ...
                                                                 TTIHelper &hb)
{
	...
	if (
	  // `ea` cross vb0
	  (vol_ea_eb0 == Sign::ZERO && vol_ea_eb2 == Sign::ZERO) ||
	  // `ea` cross vb1
	  (vol_ea_eb0 == Sign::ZERO && vol_ea_eb1 == Sign::ZERO) ||
	  // `ea` cross vb2
	  (vol_ea_eb1 == Sign::ZERO && vol_ea_eb2 == Sign::ZERO))
		return false; // intersect at a vertex -> no intersection
 ...
 * @param vtx_list list to store intersection points.
  ...
  phmap::flat_hash_set<index_t> &vtx_list)
{
	...
	Sign va_wrt_eb0 = get_v_wrt_seg(ha, va, hb, 0);
	if (va_wrt_eb0 != Sign::ZERO && va_wrt_eb0 != ori_tb)
		return false; // strictly outside `tb`
	if (va_wrt_eb0 == Sign::ZERO)
	{ // on the support line of eb0
		if (get_v_in_seg(ha, va, hb, 0))
		{ // strictly inside eb0
			g.addVertexInEdge(hb.e_id[0], ha.v_id[va]);
			inter_list.insert(ha.v_id[va]);
			vtx_list.insert(ha.v_id[va]);
			return true;
		}
	...
	Sign va_wrt_eb1 = get_v_wrt_seg(ha, va, hb, 1);
	if (va_wrt_eb1 != Sign::ZERO && va_wrt_eb1 != ori_tb)
		return false; // strictly outside `tb`
	if (va_wrt_eb1 == Sign::ZERO)
	{ // on the support line of eb1
		if (get_v_in_seg(ha, va, hb, 1))
		{ // strictly inside eb1
			g.addVertexInEdge(hb.e_id[1], ha.v_id[va]);
			inter_list.insert(ha.v_id[va]);
			vtx_list.insert(ha.v_id[va]);
			return true;
		}
	...
	Sign va_wrt_eb2 = get_v_wrt_seg(ha, va, hb, 2);
	if (va_wrt_eb2 != Sign::ZERO && va_wrt_eb2 != ori_tb)
		return false; // strictly outside `tb`
	if (va_wrt_eb2 == Sign::ZERO)
	{ // on the support line of eb2
		if (get_v_in_seg(ha, va, hb, 2))
		{ // strictly inside eb2
			g.addVertexInEdge(hb.e_id[2], ha.v_id[va]);
			inter_list.insert(ha.v_id[va]);
			vtx_list.insert(ha.v_id[va]);
			return true;
		}
 ...
 * @param hb helper for `tb`.
 * @param inter_list list to store intersection points.
 * @param copl_inter_list list to store coplanar intersection points.
 * @return true if intersections are detected.
  ...
  phmap::flat_hash_set<CoplanarEEI, Hasher> &copl_inter_list)
{
	...
	else if (is_valid_idx(ev0_in_seg)) // only v0 is in a segment of T
	{
		g.addVertexInEdge(hb.e_id[ev0_in_seg], ha.v_id[ev0]);
		inter_list.insert(ha.v_id[ev0]);
	...
	else if (is_valid_idx(ev1_in_seg)) // only v1 is in a segment of T
	{
		g.addVertexInEdge(hb.e_id[ev1_in_seg], ha.v_id[ev1]);
		inter_list.insert(ha.v_id[ev1]);
	...
	if (ev0_in_tri) // only v0 inside the triangle
	{
		g.addVertexInTriangle(hb.t_id, ha.v_id[ev0]);
		inter_list.insert(ha.v_id[ev0]);
	}
	else if (ev1_in_tri) // only v1 inside the triangle
	{
		g.addVertexInTriangle(hb.t_id, ha.v_id[ev1]);
		inter_list.insert(ha.v_id[ev1]);
	}
	...
	auto is_vb_in_ea = [&](index_t vb)
	{
		...
		if (OrientOn2D()(ha.v[ev0], ha.v[ev1], hb.v[vb], hb.t_nmax) == Sign::ZERO)
	...
	// intersection once)
	bool vb0_in_ea = is_vb_in_ea(/*vb*/ 0);
	bool vb1_in_ea = is_vb_in_ea(/*vb*/ 1);
	bool vb2_in_ea = is_vb_in_ea(/*vb*/ 2);
	...
	if (ev0_in_seg != /*eb*/ 0 && ev1_in_seg != /*eb*/ 0 && !vb0_in_ea &&
	    !vb1_in_ea && seg_seg_do_intersect(ha, ea, hb, /*eb*/ 0))
	{
		// edge `ea` cross seg 0 in `tb`
		seg0_cross =
		  add_edge_cross_coplanar_edge(glob_eb0, glob_ea, hb.t_id, copl_inter_list);
		...
		else if (is_valid_idx(ev1_in_vtx) || is_valid_idx(ev1_in_seg) || ev1_in_tri)
		{
	...
	if (ev0_in_seg != /*eb*/ 1 && ev1_in_seg != /*eb*/ 1 && !vb1_in_ea &&
	    !vb2_in_ea && seg_seg_do_intersect(ha, ea, hb, /*eb*/ 1))
	{
		// edge `ea` cross seg 1 in `tb`
		seg1_cross =
		  add_edge_cross_coplanar_edge(glob_eb1, glob_ea, hb.t_id, copl_inter_list);
	...
	if (ev0_in_seg != /*eb*/ 2 && ev1_in_seg != /*eb*/ 2 && !vb2_in_ea &&
	    !vb0_in_ea && seg_seg_do_intersect(ha, ea, hb, /*eb*/ 2))
	{
		// edge `ea` cross seg 0 in `tb`
		seg2_cross =
		  add_edge_cross_coplanar_edge(glob_eb2, glob_ea, hb.t_id, copl_inter_list);
	...
	if (vb0_in_ea)
	{
		...
		else if (is_valid_idx(ev1_in_seg) || ev1_in_tri)
		{
 ...
 * @param hb helper for `tb`.
 * @param inter_list list to store intersection points.
 * @param vtx_list list to store non-coplanar intersection points.
 * @return true if intersections are detected.
  ...
  phmap::flat_hash_set<index_t> &vtx_list)
{
	...
	if (vol_ea_eb0 == Sign::ZERO)
	{
		...
		vtx_list.insert(int_point);
	}
	// the edge intersects the tri in seg 1
	else if (vol_ea_eb1 == Sign::ZERO)
	{
		index_t int_point =
 ...
 * @param copl_inter_list coplanar intersection points
  ...
  phmap::flat_hash_set<CoplanarEEI, Hasher> &copl_inter_list)
{
	...
	for (const CoplanarEEI &c : copl_inter_list)
		if (c.is_same(ea, eb))
			return c.p;
	...
	copl_inter_list.insert(CoplanarEEI(ea, eb, ins.first));
